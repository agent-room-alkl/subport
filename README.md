...
### 价目表：分类计价 + 倍率，且账单不可被追溯改写

真实的价目表长这样（对着一个在跑的同类平台扒下来的形状）：**输入 / 输出 / 缓存读 / 缓存写四类分开计价**，
乘以**分组倍率**，再允许**账号专属倍率**覆盖分组。我们原来只有一个 `tokens` 字段、1 token = 1 额度，
这不是「表达得不好」，是**根本表达不了**——既没法对着别人报价，也没法跟客户解释一张账单。

- `model_prices`：每个模型四类费率，单位是「每百万 token 多少额度」，跟供应商公布价目的写法一致。
- `billing_groups`：分组倍率，万分之一为单位（`10000` = 1.0x，`1600` = 0.16x）。
- 用户可以有 `ratio_override`，优先于分组。缺分组、缺价目、缺用户，一律**回落到原价，不是免费**——
  「免费」这个失败模式是无声且无上限的。
- **每一行账单都把当时的费率和倍率抄一份存下来**，不是存一个指向价目表的 id。
  账单是对过去的陈述：如果只存引用，今天改价就会把上个月已经开出去的每一张账单悄悄改掉，
  而这种事要等到客户拿两份同月导出来对比才会被发现。
- 取整规则写死并有测试：**四舍五入、离零方向、只在最后统一做一次**。
  分类各自取整的话，四个 0.5 会变成 4 而不是 2。
- 供应商只报总数时，**按总数整体计费，不去猜分项**。编出来的分项看着精确，其实是假的。
